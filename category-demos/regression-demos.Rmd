...
- [ ] **G4.2** The package issues numerous error and warning messages, neither
  the triggering conditions nor values of which are explicitly tested.
    - [x] **G4.2a** Messages produced by `stop()`, `warning()`, `message()`are unique.
    - [ ] **G4.2b** Tests do not trigger every one of those messages
- [x] **G4.3** Absence of missing values form return results is explicitly tested.
...
- [x] **G4.6** Parameter recovery tests impelemented
    - [ ] **G4.6a** Many, but not all, parameter recovery tests are expected to
      succeed within a defined tolerance 
    - [ ] **G4.6b** Parameter recovery are generally only run with one single
...
- [ ] **G4.9** There appear to be no noise susceptibility tests, neither tests
  of the effect of adding trivial noise, nor tests running under different
  random seeds.

**Extended tests**

- [x] **G4.10**--**G4.12** The `lme4` package provides exemplary use of exactly
