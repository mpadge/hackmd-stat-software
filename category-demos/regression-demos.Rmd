...
- **G4.2** Appropriate error and warning behaviour of all functions should be
  explicitly demonstrated through tests.

The package issues numerous error and warning messages, neither the triggering
conditions nor values of which are explicitly tested.

- **G4.6a** *Parameter recovery tests should generally be expected to succeed
...
- **G4.6b** *Parameter recovery tests should be run with multiple random
...
- **G4.9** **Noise susceptibility tests** *Packages should test for expected
  stochastic behaviour, such as through the following conditions:*
    ...
    - **G4.9b** *Running under different random seeds or initial conditions does
