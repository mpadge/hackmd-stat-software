...
- [x] **G1.1** Statistical terminology clarified and unambiguously defined.

**Function-level Documentation**

- [ ] **G1.2** [`roxygen`](https://roxygen2.r-lib.org/) not used to documentation functions.
    - [ ] **G1.2a** Internal functions generally not documented in any form

**Supplementary Documentation**

- [x] **G1.3** Code is provided to reproduce results which form the basis of
  performance claims made in associated publications (for example, for
  `sbc()`).
- [ ] **G1.4** Code is provided to compare performance claims with alternative
...
- [ ] **G2.3** Some control of single-valued character input provides
    ...
    - [ ] **G2.3b** `tolower` is not used, nor is their explicit mention of strict case
      sensitivity
- [x] **G2.4** Provide appropriate mechanisms to convert between different *data
  types*, potentially including:
    ...
    - [x] **G2.4c** explicit conversion to character via `as.character()` (and not
      `paste` or `paste0`)
    - [x] **G2.4d**--**G2.4e** There is no explicit `factor` input, so not applicable
...
- [ ] **G3.0** Processing of names of local files explicitly documented
...
- [x] **BS2.4**--**BS2.6** Hyperparameter vectors are not submitted to `rstan`
  like most other Bayesian software, and so these standards do not apply.
...
- [x] **BS7.0** There are tests for recovery of parametric estimates of prior distribution
- [x] **BS7.1** There are tests for recovery of prior distribution in the absence of additional data
- [x] **BS7.2** There are tests for recovery of posterior distribution given specified prior and input data 
...
- [x] **BS7.4** There are tests that predicted or fitted values are on the same scale as input values.
